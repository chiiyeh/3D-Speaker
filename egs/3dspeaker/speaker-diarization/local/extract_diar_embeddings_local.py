    ...
    for rec_id in all_rec_ids:
        ...
        if not os.path.isfile(stat_emb_file):
            ...
            for segid in meta:
                ...
                if meta[segid]['stop'] - meta[segid]['start'] == 1.5 and len(feat_ls) < 15:
                    feat_ls.append(feat)
                elif meta[segid]['stop'] - meta[segid]['start'] == 1.5:
                    feat_batch = torch.concat(feat_ls, axis=0)
                    with torch.no_grad():
                        emb = embedding_model(feat_batch).cpu().numpy()
